...
class SystemInfo:
    ...
    def managing_dot_env_file() -> dict:
        ...
        about_dot_env["home"] = str(Path(__file__).parent.parent.parent.parent.parent)

        # Use a single .env file at the project root
        env_file = Path(about_dot_env["home"]) / ".env"
        ...
        if env_file.exists() and env_file.is_dir():
            inner = env_file / ".env"
            if inner.exists():
                target = Path(about_dot_env["home"]) / ".env"
        ...
        if about_dot_env["path"].exists() and about_dot_env["path"].is_file():
            print(
                f".env file already exists at: {about_dot_env['path']}. The file will be deleted and re-created to update the timestamp."
            )
    ...
    def initializing_dot_env_variables():
        ...
        dotenv.set_key(
            str(dot_env_file_info["path"]),
            "PROJECT_ROOT_DIRECTORY",
            f"{str(dot_env_file_info['home'])}",
        )
        ...
        dotenv.set_key(
            str(dot_env_file_info["path"]),
            "SYSTEM_INFORMATION",
            f"{system_info.system}",
        )
        dotenv.set_key(
            str(dot_env_file_info["path"]), "NODE_INFORMATION", f"{system_info.node}"
        )

        dotenv.load_dotenv(
            dot_env_file_info["path"], override=True
        )  # Reload to ensure new keys are available
